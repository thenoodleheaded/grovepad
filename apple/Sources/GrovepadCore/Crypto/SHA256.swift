...
/// SHA-256 in plain Swift, so GrovepadCore stays Foundation-only and the
/// checksums it computes match `sha256Hex` in cloudDocuments.ts byte for byte.
public enum SHA256 {
    ...
    public static func hex(_ bytes: [UInt8]) -> String {
        ...
        for byte in digest {
            out += String(byte >> 4, radix: 16)
            out += String(byte & 0xF, radix: 16)
        }
        return out
    }
    ...
    @inline(__always) private static func rotr(_ value: UInt32, _ count: UInt32) -> UInt32 {
