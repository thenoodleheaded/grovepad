...
| Canvas composition, global shortcuts, overlay mounting | [CanvasViewport.tsx](../src/components/canvas/CanvasViewport.tsx), [CanvasAmbient.tsx](../src/components/canvas/CanvasAmbient.tsx) | Runtime start/stop: [appRuntime.ts](../src/runtime/appRuntime.ts); canvas/store owners below | [product.css](../src/styles/product.css) | Build, lint, applicable smoke |
...
| Native desktop shell, `.grovepad` file association/icon, OS open-with | [src-tauri/src/lib.rs](../src-tauri/src/lib.rs) | Bundle/icon/fileAssociations config: [tauri.conf.json](../src-tauri/tauri.conf.json); frontend bridge: [nativeFileOpen.ts](../src/runtime/nativeFileOpen.ts); shared parse: [boardImport.ts](../src/utils/boardImport.ts); confirm-dialog state: [useBoardImportStore.ts](../src/store/useBoardImportStore.ts) | N/A — native packaging, not a UI surface | `cargo check` in `src-tauri`, `npm run tauri:dev`, open-with smoke |
...
| [WidgetCard.tsx](../src/components/widgets/WidgetCard.tsx) | Interaction name, data attribute, or child component | Shell gestures, focus, ports, fixed-state resizing, and chrome meet here |
