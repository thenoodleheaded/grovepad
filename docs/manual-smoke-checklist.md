...
- [ ] On a real iPhone, in the installed app (not Safari): pinch anywhere outside the board — nothing scales; the interface stays put. Pinch ON the board — only the camera zooms. Double-tap a card — it opens, and the page does not zoom to fit. Tap a text field inside a widget — the keyboard rises and the view does NOT lurch or scale. Press and hold a label — no magnifying loupe and no Copy/Look Up bubble appears; press and hold inside a text field — selection still works normally.
- [ ] On a real iPhone, long-press a card: ONE menu appears and it is the system action sheet, not the glass popover. Delete is red. Dismissing it by tapping outside changes nothing and leaves no menu behind. Choosing Duplicate duplicates once, not twice. Repeat on an iPad: the sheet appears anchored near the card instead of crashing.
- [ ] On a real iPhone, export a board from Settings and from the account menu, and export a document from the text editor: each opens the system share sheet, not nothing. Save one to Files and confirm the file opens. On an iPad the sheet appears as a popover and does not crash. The confirmation says "shared", never "downloaded".
- [ ] On a real iPhone, cold-launch the app from the home screen and watch the first half second: it goes straight from the home screen to the dark ground with no white flash between them.
- [ ] On a real iPhone, put the device in Airplane Mode and cold-launch: every label is in Clash Display from the first paint, never the system font.
- [ ] At phone width, open the widget library: it is a two-up grid of blocks, each showing its drawn face, with the star reachable on every block and no row clipped at the right edge.
