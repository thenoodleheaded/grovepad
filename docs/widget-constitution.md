...
*Also ratified 2026-07-13.* A `FieldDescriptor` may carry an optional `unit?: SemanticUnit` (`percent | ratio | currency | count | duration_s | date_iso | none`) on top of its raw `valueType`. This is **purely advisory** — it never blocks a connection. Grovepad's wiring stays a patchbay in the Eurorack sense: anything can patch into anything, and a nonsensical patch produces a visible, fixable, nonsensical result rather than a rejected gesture.
