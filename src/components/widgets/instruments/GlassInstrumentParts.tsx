...
export function SplitWell({value,label,tone='accent'}:{value:string|number;label:string;tone?:'accent'|'amber'|'rose'}){
  ...
  return <div className="min-w-0 text-center"><GlassWell className="relative px-2 py-2"><span className="pointer-events-none absolute inset-x-0 top-1/2 border-t border-white/[.06]"/><span className={`font-mono text-2xl font-semibold tabular-nums ${color}`}>{String(value).padStart(2,'0')}</span></GlassWell><span className="mt-1 block font-mono text-[8px] tracking-[.16em] text-neutral-600">{label}</span></div>
}
