  ...
  return (
    <div className="flex h-full flex-col gap-2">
      <div data-island="summary" className="grid grid-cols-3 gap-2">
        <Stat label="Total hours" value={Math.round(total * 100) / 100} accent="text-cyan-300" />
        <Stat label="Billable" value={Math.round(billable * 100) / 100} accent="text-emerald-300" />
        <Stat label="Amount" value={`${data.currency}${Math.round(amount * 100) / 100}`} accent="text-amber-300" />
      </div>
      <div data-island="billing" className="flex items-center justify-end gap-2  text-[8px] uppercase text-neutral-700">
      ...
      <div data-island="entries" data-floor-min-h="96" className="min-h-0 flex-1 overflow-y-auto rounded-xl border gp-hairline bg-neutral-900/25 px-2">
...
export function InventoryWidget({
  ...
  return (
    <div className="flex h-full flex-col gap-2">
      <div data-island="summary" className="grid grid-cols-3 gap-2">
        <Stat label="Total units" value={Math.round(total * 100) / 100} />
        <Stat label="Low stock" value={low.length} accent={low.length ? 'text-amber-300' : 'text-emerald-300'} />
        <Stat label="All stocked" value={allStocked ? 'Yes' : 'No'} accent={allStocked ? 'text-emerald-300' : 'text-neutral-500'} />
      </div>
      ...
      <div data-island="inventory" data-floor-min-h="96" className="min-h-0 flex-1 overflow-y-auto rounded-xl border gp-hairline bg-neutral-900/25 px-2">
        {data.items.map((item) => {
...
export function LineChartWidget({
  ...
  return (
    <div className="gp-flat-visual flex h-full flex-col gap-2">
      ...
      <div role="button" tabIndex={0} aria-label="Oscilloscope. Click to add a point" data-island="scope" data-floor-min-h="96" onClick={addAt} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();add()}}} className="gp-line-scope gp-flat-visual-own relative h-32 shrink-0 cursor-crosshair overflow-hidden border border-emerald-300/15 outline-none focus-visible:ring-1 focus-visible:ring-emerald-300/50">
      ...
      <div data-island="summary" className="grid grid-cols-3 gap-2"><Stat label="Latest" value={`${Math.round(latest * 100) / 100}${data.unit}`} accent="text-sky-300" /><Stat label="Average" value={Math.round(average * 100) / 100} /><Stat label="Maximum" value={Math.round(maximum * 100) / 100} /></div>
  ...
  return (
    <div className="gp-flat-visual flex h-full flex-col gap-2">
      ...
      <div className="gp-pie-layout grid min-h-0 flex-1 grid-cols-[minmax(96px,180px)_minmax(120px,1fr)] items-start gap-3">
        {/* A stretched circle is a lie (glass constitution, Article XVIII): the disc scales only proportionally. */}
        <div data-island="disc" data-floor-min-w="96" data-floor-min-h="96" className="flex aspect-square h-full min-h-24 w-auto min-w-24 max-w-full items-center justify-center justify-self-center self-start"><div className="relative aspect-square h-full w-full rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.25)]" style={{ background: pieGradient(data) }}>{data.mode === 'donut' && <div className="absolute inset-[25%] flex flex-col items-center justify-center rounded-full bg-neutral-950 shadow-inner"><span className=" text-[8px] uppercase text-neutral-700">Total</span><strong className=" text-sm text-neutral-200">{Math.round(total * 100) / 100}</strong></div>}</div></div>
        <div data-island="segments" data-floor-min-w="120" data-floor-min-h="96" className="min-h-0 overflow-y-auto">
